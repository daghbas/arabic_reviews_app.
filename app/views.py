...
def get_language() -> str:
    if current_user.is_authenticated and current_user.locale in LANG_STRINGS:
...
def index():
    ...
    return render_template(
        ...
        page_description=texts["description"],
    )
...
def analyze():
    ...
    if not reviews:
        ...
        reviews = (
            google_collector._sample_reviews() if source == "google" else salla_collector._sample_reviews()
        )
    ...
    return render_template(
        ...
        sample_mode=current_app.config.get("SAMPLE_MODE", False),
        lang=lang,
        page_title=texts["analysis_title"],
        page_description=texts["description"],
    )
